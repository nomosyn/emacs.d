...
(defun go-todo ()
  (interactive)
  (find-file user-todo-path))

(defun go-emacs-init ()
  (interactive)
  (find-file user-emacs-init-path))

(defun go-id ()
  (interactive)
  (find-file user-data-ids-path))

(defun go-time ()
  (interactive)
  (find-file user-time-path))
...
(defun replace_re_by_val (text pattern_value)
  "Ex:
     (replace_re_by_val \"xxx pat1, pat2 xxx\" \'(\"pat1\" . \"VAL_1\"))
      <- \"xxx VAL_1, pat2 xxx\""
  (replace-regexp-in-string (car pattern_value) (cdr pattern_value) text))

(defun replace_patterns (text patterns)
  "Ex:
     (replace_patterns \"xxx pat1, pat2 xxx\" \'((\"pat1\" . \"VAL_1\") (\"pat2\" . \"VAL_2\")))
      <- \"xxx VAL_1, VAL_2 xxx\""
  (fold 'replace_pattern text patterns))




(defun fix_ugly_char ()
  "Replace bad characters by good ones
